...
def quadratic(T,Tmin,Tmax,c):
    ...
    return Q
def diff_eqs(INP,t,tempMean,tempAmp,gamma,p1H,p2H,p12H,p1M,p2M,p12M,alpha1H,alpha2H,eiph,r):  
    '''Main equations'''
    ...
    m = gamma/g
    #print((cumulative_I12CH + cumulative_I12SH))#/(R1H +R2H +R12H))
    ...
    dE1M = lambda1M*SM - (lambda2M+lambda12M + g)*E1M - (1./eipm)*E1M
    dE2M = lambda2M*SM - (lambda1M+lambda12M + g)*E2M - (1./eipm)*E2M
    dE12M = lambda12M*SM + (lambda2M+lambda12M)*E1M + (lambda1M+lambda12M)*E2M - g*E12M - (1./eipm)*E12M 
